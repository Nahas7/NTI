...
void CLCD_vidWriteNumber(s32 Copy_u32Number)
{
	if (Copy_u32Number < 0)
	{
		CLCD_vidSendData('-');
...
void CLCD_vidSendFltNumber(f64 Copy_f64FltNumber)
{
	u32 Local_u32MaxDigits=10000;
	if(Copy_f64FltNumber<0)
	{
		CLCD_vidSendData('-');
		Copy_f64FltNumber*=(-1);
	}
	s32 Local_s32Intpart=(s32)Copy_f64FltNumber;
	Copy_f64FltNumber=Copy_f64FltNumber-Local_s32Intpart;
	CLCD_vidWriteNumber(Local_s32Intpart);
	Copy_f64FltNumber*=100000;
	Local_s32Intpart=(s32)Copy_f64FltNumber;
	if(Local_s32Intpart!=0)
	{
		CLCD_vidSendData('.');
		while(Local_s32Intpart<Local_u32MaxDigits)
		{
			Local_u32MaxDigits/=10;
			CLCD_vidWriteNumber(0);
		}
		while((Local_s32Intpart%10)==0)
		{
			Local_s32Intpart/=10;
		}
		CLCD_vidWriteNumber(Local_s32Intpart);
